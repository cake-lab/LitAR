...
class SphericalHarmonics:
    ...
    @staticmethod
    def from_sphere_points(points: pr.PointCloud, degrees=2):
        sh = SphericalHarmonics(degrees=degrees)
    ...
    def project_sphere_points(self, sphere_points: pr.PointCloud):
    ...
    def reconstruct_to_canvas(self, canvas: pr.Canvas = None) -> pr.Canvas:
        if canvas is None:
            canvas = pr.canvas_equirectangular_panorama(
                height=128)

        canvas_res = pr.canvas_equirectangular_panorama(
            height=canvas.data.shape[0])
        canvas_res.data = self.reconstruct(canvas.data)

        return canvas_res

    def vis_as_pil_image(self, canvas: pr.Canvas = None) -> PIL.Image:
        if canvas is None:
            canvas = pr.canvas_equirectangular_panorama(
                height=128)
...
def draw_equirectangular_panorama(sh, canvas_height=128):
    norm_canvas = pr.canvas_equirectangular_panorama(
